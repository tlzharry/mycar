...
# global drive_mode_instance
class DriveModeInstance:
    def __init__(self):
...
def stop_vehicle():
    drive_mode_instance.stop_signal = True
    drive_mode_instance.stop_detected_time = time.time()

def resume_vehicle():
    drive_mode_instance.stop_signal = False

def stop_rsu_vehicle():
    drive_mode_instance.stop_rsu_signal = True
...
global car2_content
car2_content = None
def scp_read_file(remote_user, remote_ip, remote_path, local_path, password):
    global car2_content
    while True:
        try:
            command = f"scp {remote_user}@{remote_ip}:{remote_path} {local_path}"
            ...
            with open(local_path, 'r') as file:
                car2_content = file.read().strip()
            
            # print(f"car2.txt:{car2_content}")
...
def start_scp_thread():
    ...
    scp_thread = threading.Thread(target=scp_read_file, args=(remote_user, remote_ip, remote_path, local_path, password))
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    #
    cam = add_camera(V, cfg, camera_type)


    def run_stop_logic(image):
        rsu_signal = stop_rsu_sign.run()
        if drive_mode_instance.stop_signal:
            return image
        else:
            processed_image = stop_line_detector.process_image(image)
            return processed_image

    # def run_stop_line_detector(image):
    #     global drive_mode_instance
    #     if drive_mode_instance.stop_detected_time is not None and (time.time() - drive_mode_instance.stop_detected_time < 3):
    #         # time_count = time.time() - drive_mode_instance.stop_detected_time
    #         # print("===暫停偵測===")
    #         # if (time_count > 1 and time_count < 1.5):
    #         #     print("time:",time.time() - drive_mode_instance.stop_detected_time)
    #         #     drive_mode_instance.stop_signal = False
    #         # print("stop_signal:",drive_mode_instance.stop_signal)
    #         # print("time:",drive_mode_instance.stop_detected_time)
    #         return image
    #     else:
    #         # print("===偵測中===")
    #         drive_mode_instance.stop_signal = False
    ...
    #
    drive_mode_instance = DriveMode(cfg.AI_THROTTLE_MULT)
...
class DriveMode:
    def __init__(self, ai_throttle_mult=0.9):
        """
